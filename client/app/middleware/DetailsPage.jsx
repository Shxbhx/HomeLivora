    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          `http://192.168.71.151:3000/admin/${id}`
        );
        
        // Get owner details using the property's userId
        const ownerResponse = await axios.get(
          `http://192.168.71.151:3000/users/${response.data.userId}`
        );
        ...
        const savedResponse = await axios.get(
          `http://192.168.71.151:3000/users/${userId}/saved`
        );
  ...
  const handleSave = async () => {
    try {
      ...
      if (!isSaved) {
        ...
        await axios.post(`http://192.168.71.151:3000/users/${userId}/saved/${id}`);
      } else {
        // Remove from backend
        console.log("Removing property from backend...");
        await axios.delete(`http://192.168.71.151:3000/users/${userId}/saved/${id}`);
      }
    } catch (error) {
